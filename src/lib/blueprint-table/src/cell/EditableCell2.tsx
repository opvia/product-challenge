...
import { CellProps, Cell, Draggable } from "@blueprintjs/table";
...
export interface EditableCell2Props extends Omit<CellProps, "onKeyDown" | "onKeyUp"> {
    ...
    /**
     * A listener that is triggered if the user starts editing the cell.
     */
    onEdit?: (value: string, rowIndex?: number, columnIndex?: number) => void;

    /**
     * Value to show when editing
     */
    onEditValue?: string
}
...
export class EditableCell2 extends React.Component<EditableCell2Props, EditableCell2State> {
    ...
    public componentDidUpdate(prevProps: EditableCell2Props) {
    ...
    public shouldComponentUpdate(nextProps: EditableCell2Props, nextState: EditableCell2State) {
        return (
    ...
    private handleEdit = () => {
        this.setState({ isEditing: true, dirtyValue: this.props.onEditValue || this.state.savedValue });
        this.invokeCallback(this.props.onEdit, this.state.savedValue || '');
    };
